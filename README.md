...
--enable-nw                 設定用ネットワークインターフェースを有効化します。  
--bind-addr                 設定用ネットワークインターフェースで待ち受けるIPアドレスを設定します。  
--bind-port                 設定用ネットワークインターフェースで待ち受けるポートを設定します。  
...
--sleep-time                     メインループでの休止時間を指定します。
                                 小さくすると音声が途切れにくくなりますが、負荷が高くなります。  
```
...
ネットワーク周りについて、現在の状況は以下の通りです。  
* macOS
    * ファイアウォールが有効になっているとき、ローカルホスト以外でネットワーク通信を待ち受けることはできません。  

その他、Windowsの場合sleepの粒度が1msecである都合、更新タイミング等での1000以下の指定は切り捨てになるほか、意図した挙動になりません。
